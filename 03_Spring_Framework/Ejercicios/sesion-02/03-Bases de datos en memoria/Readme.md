...
Necesitarás la dependencia de H2 en tu archivo `pom.xml` para poder utilizar esta base de datos. La configuración típica para H2 en Spring Boot se ve así:
...
Ahora, considera el modelo de datos de una entidad `Product` que incluye campos como id, name, description, country, y visible. Un ejemplo de archivo ``data.sql`` podría ser:
